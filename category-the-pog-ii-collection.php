...
<div id="main-content">
  <div class="product-menu-wrap product-tabs">
    <div class="product-menu-wrap-main">
      <div class="product-name-sec"><?php the_field('pog_hero_title', 'option'); ?></div>
  ...
  <div class="product-tab-content active" id="product-overview">
    <div class="meet-summery-wrap hero-section">
      <div class="meet-summery-lt hero-section-left">
        <?php
          $herotitle = get_field('pog_hero_title','option');
          echo !empty($herotitle)?'<h1>'.$herotitle.'</h1>':''; 
          $herosubtitle = get_field('pog_hero_sub_title','option');
          echo !empty($herosubtitle)?'<h2>'.$herosubtitle.'</h2>':''; 
        ?>
      </div>
      <div class="meet-summery-rt hero-section-right">
        <?php 
        $hero_imagevideo = get_field('pog_hero_imagevideo','option'); 
        if($hero_imagevideo == 'video'){
          $herovideo = get_field('pog_hero_video','option'); 
          if(!empty($herovideo)){
            echo '<div class="embed-container">'.$herovideo.'</div>';
          }
        }else{
          $heroimage = get_field('pog_hero_image','option'); 
          if(!empty($heroimage)){
    ...
    <?php $cta = get_field('pog_call_to_action','option');
    ...
    <div class="overview-highlights-wrap">
      ...
      $wpb_all_query = new WP_Query(array('post_type'=>'product', 'post_status'=>'publish', 'category_name'=>'The Pog ii Collection', 'order' => 'ASC', 'posts_per_page'=>-1)); ?>
    ...
    <?php 
    $display_image_gallery = get_field('pog_display_image_gallery','option');
      ...
      <div class="image-gallery">
        ...
        <ul class="gallery-slides">
          ...
          foreach ($display_image_gallery['image_gallery'] as $key => $value) {
           ...
           if (strpos($image_gallery, "pog-img2.jpg") == 0 && strpos($image_gallery, "pog-img1.jpg") == 0 && strpos($image_gallery, "pog-hero.jpg") == 0 ) {
            ?>
    ...
    <div class="product-resource-wrap-main main-resource-wrap">
      ...
      <div class="resource-wrap">
        <div class="resource-inner">
          <h2>Pro Resources</h2>
          <?php
          if( have_rows('pog_resources', 'option') ):
            while ( have_rows('pog_resources', 'option') ) : the_row(); ?>
              <div class="pdf-item">
      ...
      <?php if( get_field('pog_manuals', 'option') ): ?>
        <div class="resource-wrap">
          <div class="resource-inner">
          <h2>Manuals</h2>
          <?php
          if( have_rows('pog_manuals', 'option') ):
            while ( have_rows('pog_manuals', 'option') ) : the_row(); ?>
            <div class="pdf-item">
              <div class="pdf-item-lt"><?php the_sub_field('title'); ?></div>
              <div class="pdf-item-rt"><a href="<?php the_sub_field('pdf'); ?>" target="_blank"><i class="fas fa-download"></i> PDF</a></div>
            </div><!--pdf-item-->
